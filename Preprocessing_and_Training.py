...
from Utility_Classes.model_perfomence import ModelFinder
...
logfile =  open(r'Logs/training_log.log', 'a+')
...
#Save the models
trained_model_names = [str(results[i][0].__class__).split(".")[-1].strip("'>") for i in range(len(results))]
trained_models = [results[i][0] for i in range(len(results))]

for name, model in zip(trained_model_names, trained_models):
    pickle.dump(model, open(r"Saved_Models/"+name+".pkl", 'wb'))
